...
This server bridges to Shipyard Bay API for sandboxed Python execution.
    ...
    async def execute_python(
        ...
        try:
            response = await self.client.post(
                f"/ship/{session_id}/python/execute",
                json={"code": code},
            )
    ...
    async def handle_request(self, request: dict) -> dict:
        ...
        if method == "tools/call":
            ...
            if tool_name == "python_exec":
                ...
                result = await self.execute_python(code, session_id)
...
async def main():
    ...
    server = PythonExecServer(bay_url)
    await server.run()
